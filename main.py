    ...
    for stockA in args:
        for stockB in args:
            if stockA is stockB:
                ...
                diversification.append(2 * (stockA["weight"]) * (stockB["weight"]) * co_variance(stockA, stockB))
...
def calculate_portfolio(portfolio, round_to_decimal=4):

    variance_of_portfolio = round(math.sqrt(portfolio_variance(portfolio)), round_to_decimal)
